namespace SocoShop.Page
{
    ...
    public class Login : CommonBasePage
    {
        ...
        protected UserInfo checklogin(String logname, String loginpass)
        {
            UserInfo user = UserBLL.CheckUserLogin(logname, loginpass);
            return user;
        }
        ...
        protected override void PostBack()
        {
            ...
            {
                UserInfo user = checklogin(loginName, loginPass);//UserBLL.CheckUserLogin(loginName,loginpass);
                if (user.ID <= 0)
